...
def get_best_option_index(options, requested):
    best_option = min(
        options,
        key=lambda option: levenshtein_distance_nounicode(requested, option)
    )
...
def select_option(docxml, name, option_to_select):
    ...
    the selection depends on the presence of a <w:result w:val="$INDEX"/> field
    inside this checkBox element
    """
    start_selector = './/w:name[@w:val="%s"]' % name
